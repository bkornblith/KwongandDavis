import { formatDateTime, formatElapsed } from '@sanity/cli-core/dates';
...
import { arrayifyCors, arrayifyDataset, arrayifyFunction, arrayifyRobot, arrayifyRole, arrayifyStudio, arrayifyWebhook, studioHost, studioUrl, } from './resources-formatting.js';
...
export function formatStudioUrls(resources) {
    if (!resources)
        return [];
    const lines = [];
    for (const res of resources) {
        if (res.type !== SANITY_STUDIO)
            continue;
        const host = studioHost(res.resolvedParameters ?? res.parameters);
        if (host)
            lines.push(`Studio ${styleText(['bold', 'green'], res.name)} ${studioUrl(host)}`);
    }
    return lines;
}
...
export function formatStackInfo(stack, isCurrentStack = false) {
    ...
    if (stack.recentOperation) {
        ...
        if (op.completedAt && op.createdAt) {
            ...
            parts.push(styleText('yellow', `(${formatElapsed(duration)})`));
        }
